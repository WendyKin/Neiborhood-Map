...
var ViewModel = function() {
  ...
  //每点击一次汉堡菜单，menuHide值在true和false间切换
  self.hbClick = function() {
    self.menuHide(!self.menuHide());
  };
  //当menuHide为true时，给容器增加menu-hide的class；为false时去掉
  self.menuhidden = ko.pureComputed(function() {
    return self.menuHide() ? "menu-hide" : "";
  }, self);
  ...
  self.listShow = ko.computed(function() {
    return ko.utils.arrayFilter(self.locList(), function(location) {
      //回调函数，使用指定函数测试所有元素，并创建一个包含所有通过测试的元素的新数组
      if (infoWindow.getIsOpen()) {
        infoWindow.close(); // 如果 infowindow 已经打开，则关闭它。
	  }
	  //map.setZoomAndCenter(6, [113.0377655, 34.45993805]); 
	 // map.setFitView();
...
function mkClick() {
  ...
  var weatherUrl =
    ...
    position +
    "&key=30962604458e42ffbaaf4c54ec3e27d4";
